...
const quantities: Quantities = {
	...
	temperature: [
		...
		{
			name: 'fahrenheit',
			toSelf: '((9/5)*(x-273.15))+32',
			toBase: '((5/9)*(x-32))+273.15',
			short: ['°F', 'fh', 'f'],
			evalRequired: true
		}
	],
...
export default {
	async run(client: YClient, interaction: Discord.ChatInputCommandInteraction<"cached">) {
		...
		if (subCmd === 'help') {
			...
			const starters = args.slice(0, args.indexOf(separator)).join(' ').split(',').map((starter: string) => {
			...
			if (starters[0]?.unit.evalRequired) {
				...
				absolute = starters.map((starter: any) => starter.amount * starter.unit.value).reduce((a, b) => a + b, 0);
			}
