...
export default {
	async run(client: YClient, interaction: Discord.ChatInputCommandInteraction<"cached">) {
		...
		const embed = new client.embed()
			...
			.setDescription(`List of commands that have been used. Total amount of commands used since last restart: **${client.commands.filter(x => x.uses).map(x => x.uses).reduce((a, b) => a + b, 0)}**`)
		...
		embed.addFields(
			{name: 'Node.js', value: [
				`**RAM:** ${client.formatBytes(process.memoryUsage().heapUsed, 2, 1000)}**/**${client.formatBytes(os.freemem(), 2, 1000)}`,
			...
			{name: 'System', value: [
				...
				`**RAM:** ${client.formatBytes(os.totalmem(), 2, 1000)}`,
