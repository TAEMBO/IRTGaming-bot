...
export default {
	async run(interaction: TInteraction) {
        ...
        if (subCmd === 'all') {
            ...
            const data: number[] = JSON.parse(fs.readFileSync(path.resolve(`../databases/${subCmd.toUpperCase()}PlayerData.json`), 'utf8')).slice(interaction.client.config.statsGraphSize);
            ...
            if (FSdss.slots.used === FSdss.slots.capacity) {
                Color = interaction.client.config.embedColorRed;
            } else if (FSdss.slots.used > 9) Color = interaction.client.config.embedColorYellow;
            ...
            await interaction.reply({ embeds: [embed], files: [Image] }).catch(() => interaction.channel?.send({ embeds: [embed], files: [Image] }));
        };
    },
    data: cmdBuilderData
};
