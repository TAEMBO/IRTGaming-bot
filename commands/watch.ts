...
export default {
	async run(interaction: TInteraction) {
        ...
        const wlData = await interaction.client.watchList._content.findById(name);

        ({
            add: async () => {
                const reason = interaction.options.getString('reason', true);
                if (!wlData) {
                    await interaction.client.watchList._content.create({ _id: name, reason });
            ...
            remove: async () => {
                if (wlData) {
                    await interaction.client.watchList._content.findByIdAndDelete(name);
                    interaction.reply(`Successfully removed \`${name}\` from watchList`);
                } else interaction.reply(`\`${name}\` doesn't exist on watchList`);
            },
            view: async () => {
                fs.writeFileSync('../databases/watchListCache.json', JSON.stringify(await interaction.client.watchList._content.find(), null, 2));
