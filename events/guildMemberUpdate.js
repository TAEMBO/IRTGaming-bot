module.exports = {
    ...
    execute: async (client, member) => {
        const mp_tf = (await client.guilds.cache.get(client.config.mainServer.id).roles.fetch(client.config.mainServer.roles.trustedfarmer)).members.map(e=>`<@${e.user.id}>`).join("\n") || " ";
