module.exports = {
    ...
    execute: async (client, member) => {
        ...
        client.channels.cache.get('718555644801712200')?.messages?.fetch('980240957167521863').then((msg)=>{msg.edit({content: `<@&${client.config.mainServer.roles.trustedfarmer}>: ${mp_tf.size}\n${mp_tf.map(e=>`<@${e.user.id}>`).join("\n") || " "}`, allowedMentions: {roles: false}})})
    }
}
