...
export default async (client: YClient, oldState: Discord.VoiceState, newState: Discord.VoiceState) => {
    ...
    const embed = new client.embed()
        ...
        .setThumbnail(newState.member.user.avatarURL({ extension: 'png', size: 2048}) || newState.member.user.defaultAvatarURL);
    ...
    if (oldState.channelId == null && newState.channelId != null) { // Joined VC
        ...
        embed.addFields({name: '🔹 Old Channel', value: `<#${oldState.channelId}>`}, {name: '🔹 New Channel', value: `<#${newState.channelId}>`})
        channel.send({embeds: [embed]});
    }
}