...
client.init().then(() => {
	...
	client.on("ready", async () => {
		await client.guilds.fetch(client.config.mainServer.id).then(async guild => {
			...
			if (client.config.botSwitches.registerCommands) guild.commands.set(client.registry).catch(e => console.log(`Couldn't register commands bcuz: ${e}`));
		});
...
setInterval(async () => {
	const now = Date.now();
	client.reminders._content.find().then(y => y.filter(x => now > x.time).forEach(async reminder => {
	...
	client.punishments._content.find().then(y => y.filter(x => x.endTime && x.endTime <= now && !x.expired).forEach(async punishment => {
...
if (client.config.botSwitches.FSLoop) setInterval(async () => {
	await FSLoop(client, client.tokens.fs.ps.dss, client.tokens.fs.ps.csg, '778848112588095559', '1023653622971637800', 'PS');
	await FSLoop(client, client.tokens.fs.pg.dss, client.tokens.fs.pg.csg, '778848112588095559', '1023653628055134250', 'PG');
	// await FSLoop(client, client.tokens.fs.mf.dss, client.tokens.fs.mf.csg, '982143077554720768', '985586585707900928', 'MF');
}, 30000);

// YouTube upload nofitcations loop
if (client.config.botSwitches.YTLoop) setInterval(async () => {
	await client.YTLoop('UCQ8k8yTDLITldfWYKDs3xFg', 'Daggerwin');
	await client.YTLoop('UCLIExdPYmEreJPKx_O1dtZg', 'IRTGaming');
	await client.YTLoop('UCguI73--UraJpso4NizXNzA', 'Machinery Restorer');
	await client.YTLoop('UCuNIKo9EMJZ_FdZfGnM9G1w', 'Tom Pemberton Farm Life');
	await client.YTLoop('UCKXa-FhJpPrlRigIW1O0j8g', 'Alan McPhearson');
	await client.YTLoop('UCWYXg1sqtG9NalK5ZGt4ITA', 'Chainsaw100');
}, 300000);
