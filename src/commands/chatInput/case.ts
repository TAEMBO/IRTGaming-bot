    ...
    async run(interaction) {
        ...
        switch (interaction.options.getSubcommand()) {
            case "view": {
                ...
                const cancelledBy = punishment.expired
                    ? await interaction.client.punishments.data.findOne({ cancels: punishment.id })
                    : null;
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        {
                            name: "🔹 User",
                            value: `${punishment.member.tag}\n<@${punishment.member._id}> \`${punishment.member._id}\``,
                            inline: true,
                        },
                        {
                            name: "🔹 Moderator",
                            value: `<@${punishment.moderator}> \`${punishment.moderator}\``,
                            inline: true,
                        },
            ...
            case "member": {
                ...
                const userPunishments = userPunishmentsData
                    ...
                    .map(punishment => ({
                        ...
                        value: [
                            ...
                            punishment.expired
                                ...
                                : "",
                            punishment.cancels ? `\n> __Overwrites Case #${punishment.cancels}__` : "",
                        ].join(""),
                    }));
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(x =>
            x
                ...
                .addIntegerOption(x => x.setName("id").setDescription("The ID of the case").setRequired(true)),
        )
        .addSubcommand(x =>
            x
                .setName("member")
                .setDescription("View all of a member's cases")
                .addUserOption(x => x.setName("user").setDescription("The member whose cases to view").setRequired(true))
                .addIntegerOption(x => x.setName("page").setDescription("The page number (if multiple case pages)").setRequired(false)),
        )
        .addSubcommand(x =>
            x
                .setName("update")
                ...
                .addIntegerOption(x => x.setName("id").setDescription("The ID of the case").setRequired(true))
                .addStringOption(x => x.setName("reason").setDescription("The new reason for the case").setRequired(true)),
        )
