...
export default new Command<"chatInput">({
    async run(interaction) {
        ...
        switch (interaction.options.getString("questions", true)) {
            case "staff":
                ...
                await interaction.reply({
                    ...
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(
                                [
                                    `If a player is causing problems on a server, ${
                                        isFromTicket
                                            ? "let us know"
                                            : `don"t hesitate to send a report to ${fsServers
                                                  .getPublicAll()
                                                  .map(([_, x]) => `<#${x.channelId}>`)
                                                  .join(" or ")}`
                                    } with:`,
                ...
                await interaction.reply({
                    ...
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setFields(
                                ...fsServers.getPublicAll().map(([_, x]) => ({ name: x.fullName, value: "- " + x.todo.join("\n- ") })),
                            ),
                    ],
                });
                ...
                await interaction.reply({
                    ...
                    embeds: [
                        new EmbedBuilder()
                            .setColor(interaction.client.config.EMBED_COLOR)
                            .setTitle(
                                'Please note that servers may "ghost" and not show up until you\'ve refreshed your MP menu several times.',
                            )
    ...
    data: new SlashCommandBuilder()
        ...
        .addUserOption(x => x.setName("user").setDescription("The optional user to notify of with this FAQ")),
});
