...
const cmdBuilderData = new SlashCommandBuilder().setName("mf").setDescription("MF management");

for (const [serverAcro, server] of fsServers.getPrivateAll())
    cmdBuilderData.addSubcommandGroup(x =>
        x
            ...
            .addSubcommand(x =>
                x
                    ...
                    .addUserOption(x => x.setName("member").setDescription("The member to add or remove a role from").setRequired(true))
                    .addStringOption(x =>
                        x.setName("role").setDescription("The role to add or remove").setRequired(true).setAutocomplete(true),
                    ),
            )
            .addSubcommand(x =>
                x
                    ...
                    .addUserOption(x =>
                        x.setName("member").setDescription("The member to add or remove the MF Farm Owner role from").setRequired(true),
                    ),
            )
            .addSubcommand(x =>
                x
                    ...
                    .addStringOption(x => x.setName("role").setDescription("The role to rename").setRequired(true).setAutocomplete(true))
                    .addStringOption(x =>
                        x
                            .setName("name")
            ...
            .addSubcommand(x =>
                x
                    ...
                    .addStringOption(x =>
                        x.setName("channel").setDescription("The channel to rename").setRequired(true).setAutocomplete(true),
                    ),
            )
            .addSubcommand(x =>
                x
            ...
            .addSubcommand(x => x.setName("apply").setDescription("Send the Google Form to apply to be an MF Farm Owner")),
    );
    ...
    async autocomplete(interaction) {
        ...
        switch (interaction.options.getSubcommand()) {
            ...
            case "rename-channel": {
                ...
                const activeFarmChannels = interaction.client
                    .mainGuild()
                    .channels.cache.filter(x => regExp.test(x.name) && x.parentId === serverObj.category);
                const archivedFarmChannels = interaction.client
                    .mainGuild()
                    .channels.cache.filter(
                        x => regExp.test(x.name) && x.parentId === interaction.client.config.mainServer.categories.archived,
                    );
        ...
        switch (interaction.options.getSubcommand()) {
            case "member": {
                ...
                if (!validFarmIds.includes(roleId))
                    return await interaction.reply("You need to select a valid MF Farm role from the list provided!");

                if (!member)
                    return await interaction.reply({
                        content: "You need to select a member that's in this server",
                        ephemeral: true,
                    });
                ...
                if (!member.roles.cache.has(roleId)) {
                    ...
                    return await interaction.reply({
                        embeds: [
                            new EmbedBuilder()
                                .setDescription(
                                    `${member} (${member.user.tag}) has been given the <@&${serverObj.roles.member}> and <@&${roleId}> roles.`,
                                )
                ...
                (
                    ...
                    .on("collect", async int => {
                        switch (int.customId) {
                            case "confirm": {
                                const rolesToRemove =
                                    onMFFarms(member, serverAcro).length === 1 ? [roleId, serverObj.roles.member] : [roleId];
                                ...
                                await int.update({
                                    embeds: [
                                        new EmbedBuilder()
                                            .setDescription(
                                                `${member} (${member.user.tag}) has been removed from the ${rolesToRemove.map(x => `<@&${x}>`).join(" and ")} role(s).`,
                                            )
                                            .setColor(interaction.client.config.EMBED_COLOR),
                                    ],
                                    components: [],
            ...
            case "owner": {
                ...
                const msg = await interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setDescription(
                                `This member already has the <@&${serverObj.roles.farmOwner}> role, do you want to remove it from them?`,
                            )
                            .setColor(interaction.client.config.EMBED_COLOR),
                    ],
                    components: ACK_BUTTONS,
                    ...
                    if (int.customId === "confirm") {
                        ...
                        await int.update({
                            embeds: [
                                new EmbedBuilder()
                                    .setDescription(
                                        `${member} (${member.user.tag}) has been removed from the <@&${serverObj.roles.farmOwner}> role`,
                                    )
                                    .setColor(interaction.client.config.EMBED_COLOR),
                            ],
                            components: [],
            ...
            case "rename-channel": {
                ...
                if (!channel || !new RegExp(serverAcro).test(channel.name))
                    return await interaction.reply("You need to select a channel from the list provided!");
            ...
            case "archive": {
                ...
                if (channelisActive) {
                    // Channel currently active, change to archived
                    await channel.edit({
                        parent: archived,
                    ...
                    return await interaction.reply(`${channel} successfully set to archived`);
                }

                // Channel currently archived, change to active
                await channel.edit({
