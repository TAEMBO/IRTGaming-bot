...
export default new Command<"chatInput">({
    async run(interaction) {
        ...
        if (interaction.options.getSubcommand() === "view") {
            ...
            const pronounBool = (you: string, they: string) => {
                // takes 2 words and chooses which to use based on if user did this command on themself
            ...
            if (!userData)
                return await interaction.reply(
                    `${pronounBool("You", "They")} currently don't have a level, send some messages to level up.`,
                );
            ...
            const levelDifference =
                interaction.client.userLevels.algorithm(userData.level + 1) - interaction.client.userLevels.algorithm(userData.level);
        ...
        await interaction.reply({
            ...
            embeds: [
                new EmbedBuilder()
                    ...
                    .setDescription(
                        `A total of **${messageCountsTotal.toLocaleString("en-US")}** messages have been recorded in this server.`,
                    )
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(x =>
            x
                ...
                .addUserOption(x => x.setName("member").setDescription("Member whose rank to view").setRequired(false)),
        )
        .addSubcommand(x => x.setName("leaderboard").setDescription("View top 10 users in the ranking system")),
});
