...
export default new Command<"chatInput">({
    async run(interaction) {
        ...
        async function promptDeletion(
            ...
            (await (int.isChatInputCommand() ? int.reply(intOptions) : int.update(intOptions)))
                ...
                .on("collect", async int => {
                    if (int.customId === "cancle") return int.update(rplText("Command manually canceled"));
        ...
        switch (interaction.options.getSubcommand()) {
            ...
            case "delete": {
                ...
                if (userReminders.length === 0)
                    return await interaction.reply({ content: "You have no active current reminders", ephemeral: true });

                if (userReminders.length === 1) return await promptDeletion(userReminders[0], interaction);

                const selectMenu = new StringSelectMenuBuilder().setCustomId("reminders").setPlaceholder("Choose a reminder to delete");
                ...
                for (const [i, x] of userReminders.entries()) {
                    ...
                    embed.addFields({
                        name: `#${index}`,
                        value: [`> Content: \`${x.content}\``, `> Time to remind: ${formatTime(x.time)}`].join("\n"),
                    });
                }
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(x =>
            x
                ...
                .addStringOption(x => x.setName("what").setDescription("The content of the reminder").setRequired(true))
                .addStringOption(x => x.setName("when").setDescription("The time to remind from now").setRequired(true)),
        )
        .addSubcommand(x => x.setName("delete").setDescription("Delete an active reminder")),
});
