...
const cmdBuilderData = new SlashCommandBuilder()
    ...
    .addSubcommand(x => x.setName("all").setDescription("Server stats for all servers"))
    .addSubcommand(x =>
        x
            .setName("playertimes")
...
for (const [serverAcro, { fullName }] of fsServers.entries())
    cmdBuilderData.addSubcommand(x => x.setName(serverAcro).setDescription(`${fullName} server stats`));
    ...
    async run(interaction) {
        ...
        if (subCmd === "all") {
            ...
            await Promise.allSettled(
                fsServers.entries().map(async ([serverAcro, server]) => {
            ...
            const sortedData = playersData.sort(
                (a, b) =>
                    interaction.client.playerTimes.getTimeData(b).reduce((x, y) => x + y[1].time, 0) -
                    interaction.client.playerTimes.getTimeData(a).reduce((x, y) => x + y[1].time, 0),
            );
            const player = interaction.options.getString("name");

            const leaderboard = (data: PlayerTimesDocument[], isFirstField: boolean) =>
                data
                    .map((x, i) =>
                        [
                            ...
                            " - ",
                            formatTime(interaction.client.playerTimes.getTimeData(x).reduce((x, y) => x + y[1].time, 0) * 60 * 1000, 3, {
                                commas: true,
                            }),
                        ].join(""),
                    )
                ...
                return await interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(
                                `Top 50 players with the most time spent on IRTGaming FS22 servers since ${interaction.client.config.PLAYERTIMES_START_DATE}`,
                            )
                            .addFields(
            ...
            if (!playerData)
                return await interaction.reply(
                    `No data found with that name. [Find out why](${interaction.client.config.resources.statsNoDataRedirect})`,
                );
                    ...
                    new EmbedBuilder()
                        ...
                        .setTitle(
                            [
                                ...
                                isMPStaff(interaction.member) && playerData.discordid
                                    ? `Discord user ID - \`${playerData.discordid}\``
                                    : "",
                            ].join("\n"),
                        )
            ...
            const serverSlots = `${dss.slots.used}/${dss.slots.capacity}`;
            const serverTimeHrs = Math.floor(dss.server.dayTime / 3600 / 1000)
                .toString()
                .padStart(2, "0");
            const serverTimeMins = Math.floor((dss.server.dayTime / 60 / 1000) % 60)
                .toString()
                .padStart(2, "0");
            ...
            if (!players.some(x => x.isAdmin) && interaction.client.fsCache[subCmd].lastAdmin)
                embed.setTimestamp(interaction.client.fsCache[subCmd].lastAdmin).setFooter({ text: "Admin last on" });

            await interaction.reply({
                embeds: [embed],
                files: [new AttachmentBuilder(img.toBuffer("image/png"), { name: "FSStats.png" })],
            });
        }
    },
    data: cmdBuilderData,
