...
export default new Command<"chatInput">({
    async run(interaction) {
        ...
        if (!["warn", "mute"].includes(punishment.type) && hasRole(interaction.member, "discordHelper"))
            return await youNeedRole(interaction, "discordModerator");
    ...
    data: new SlashCommandBuilder()
        ...
        .addIntegerOption(x => x.setName("caseid").setDescription("The ID of the punishment to overwrite").setRequired(true))
        .addStringOption(x => x.setName("reason").setDescription("The reason for overwriting the punishment").setRequired(false))
