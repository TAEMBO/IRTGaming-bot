    ...
    async run(interaction) {
        ...
        switch (interaction.options.getSubcommand()) {
            case "add": {
                ...
                if (!wlData) {
                    await interaction.client.watchList.data.create({
                        _id: name,
                        reason,
                        isSevere: severity === "ban" ? true : false,
                    });
                    await interaction.reply(
                        `Successfully added \`${name}\` who needs to be **${severity === "ban" ? "banned" : "watched over"}** with reason \`${reason}\``,
                    );
            ...
            case "view": {
                await interaction.reply({
                    files: [
                        new AttachmentBuilder(Buffer.from(JSON.stringify(await interaction.client.watchList.data.find(), null, 2)), {
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(x =>
            x
                ...
                .addStringOption(x => x.setName("username").setDescription("The player name to add").setRequired(true))
                .addStringOption(x => x.setName("reason").setDescription("The reason for adding the player").setRequired(true))
                .addStringOption(x =>
                    x
                        .setName("severity")
                        .setDescription("Whether this player needs to be banned or watched over")
                        .addChoices({ name: "Needs to be banned", value: "ban" }, { name: "Needs to be watched over", value: "watch" })
                        .setRequired(true),
                ),
        )
        .addSubcommand(x =>
            x
                ...
                .addStringOption(x => x.setName("username").setDescription("The player name to remove").setRequired(true)),
        )
        .addSubcommand(x => x.setName("view").setDescription("View the full watchList"))
        .addSubcommand(x => x.setName("subscription").setDescription("Manage your subscription to watchList notifications")),
});
