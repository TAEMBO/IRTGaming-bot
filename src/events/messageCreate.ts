...
import { fsServers, hasProfanity, isDCStaff, isMPStaff, log, tempReply } from "#util";

export default new Event({
    name: Events.MessageCreate,
    async run(message) {
        if (
            (!message.client.config.toggles.commands && !message.client.config.devWhitelist.includes(message.author.id)) ||
            message.system ||
            message.author.bot ||
            !message.inGuild()
        )
        ...
        if (message.mentions.roles.has(message.client.config.mainServer.roles.mpStaff)) {
            ...
            message.channel
                ...
                .on("collect", async collected => {
                    ...
                    await message.client.punishments.addPunishment(
                        "mute",
                        collected.author.id,
                        "Automod; Misuse of staff ping",
                        message.author,
                        message.member,
                        { time: "10m" },
                    );
        ...
        // RepeatedMessages
        const isWhitelisted = message.client.config.whitelist.bannedWords.some(x =>
            [message.channelId, message.channel.parentId].includes(x),
        );
        ...
        // MF mod voting
        if (fsServers.getPrivateAll().some(x => x[1].modSuggestions === message.channelId) && message.content.includes("http")) {
        ...
        if (
            ...
            const staffTag =
                (message.member!.displayName.indexOf(" | ") ?? NaN) < 0 ? undefined : message.member!.displayName.indexOf(" | ");
