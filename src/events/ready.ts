    ...
    async run(client) {
        ...
        if (client.config.toggles.registerCommands) {
            await client.application.commands
                .set([...client.chatInputCommands.map(x => x.data.toJSON()), ...client.contextMenuCommands.map(x => x.data.toJSON())])
        ...
        for (const [code, inv] of await guild.invites.fetch())
            client.inviteCache.set(code, {
                uses: inv.uses ?? 0,
        ...
        cron.schedule(
            ...
            async date => {
                ...
                const yesterday = client.dailyMsgs.cache.find(x => x._id === formattedDate - 1) ?? {
                    _id: formattedDate - 1,
                    count: 0,
                };
        ...
        if (client.config.toggles.ytLoop)
            setInterval(async () => {
                for (const channel of client.config.ytChannels) {
                    ...
                    client.ytCache[channel.id] = latestVid["yt:videoId"]._text;
                    await client
                        .getChan("videosAndLiveStreams")
                        .send(`**${channel.name}** just uploaded a new video!\n${latestVid.link._attributes.href}`);
                }
            }, 1_800_000);
    },
});
