...
import { DSSExtension, DSSFile, type DSSResponse, Feeds, filterUnused, type PlayerUsed } from "farming-simulator-types/2022";
...
export async function fsLoop(client: TClient, watchList: WatchListDocument[], server: FSServer, serverAcro: string) {
    ...
    const serverStatusEmbed = (status: string) =>
        new EmbedBuilder().setTitle(`${serverAcroUp} now ${status}`).setColor(client.config.EMBED_COLOR_YELLOW).setTimestamp();
    ...
    const dss = await (async () => {
        const res = await fetch(server.url + Feeds.dedicatedServerStats(server.code, DSSExtension.JSON), init).catch(err =>
            log("Red", `${serverAcroUp} DSS ${err.message}`),
        );
    ...
    const csg = await (async () => {
        ...
        const res = await fetch(server.url + Feeds.dedicatedServerSavegame(server.code, DSSFile.CareerSavegame), init).catch(err =>
            log("Red", `${serverAcroUp} CSG ${err.message}`),
        );
    ...
    const toThrottle = (() => {
        // Throttle Discord message updating if no changes in API data
    ...
    const stats = {
        ...
        ingameTime: dss.server.dayTime
            ? [
                  Math.floor(dss.server.dayTime / 3_600 / 1_000)
                      .toString()
                      .padStart(2, "0"),
                  ":",
                  Math.floor((dss.server.dayTime / 60 / 1_000) % 60)
                      .toString()
                      .padStart(2, "0"),
              ].join("")
            : "`unavailable`",
        timescale: csg.settings?.timeScale._text ? parseFloat(csg.settings.timeScale._text) + "x" : "`unavailable`",
        playTime: (() => {
            const time = parseInt(csg.statistics.playTime._text);

            return time
                ? [(time / 60).toLocaleString("en-US"), "hrs (", formatTime(time * 60 * 1_000, 3, { commas: true }), ")"].join("")
                : "`unavailable`";
        })(),
    ...
    for (const player of newPlayerList.filter(x => oldPlayerList.some(y => x.isAdmin && !y.isAdmin && y.name === x.name))) {
        if (!client.whitelist.cache.includes(player.name) && !client.fmList.cache.includes(player.name) && !server.isPrivate) {
            await client.getChan("juniorAdminChat").send({
                embeds: [
                    new EmbedBuilder()
            ...
            await logChannel.send({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(
                            `\`${player.name}\`${decorators(player)} logged in as admin on **${serverAcroUp}** at <t:${now}:t>`,
                        ),
                ],
            });
    }
    ...
    for (const player of joinedPlayers) {
        ...
        if (inWl) {
            const filterWLPings = client.watchListPings.cache.filter(
                x => !client.mainGuild().members.cache.get(x)?.roles.cache.has(client.config.mainServer.roles.loa),
            );
